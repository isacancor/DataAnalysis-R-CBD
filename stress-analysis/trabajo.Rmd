...
Por ejemplo, el Cluster 1 tiene muchas personas con estrés alto, y el Cluster 2 tiene muchas personas con estrés medio y bajo, lo cual puede ayudarnos a averiguar cual de las variables influye más. Para ello:
...
### 5.5.1 Cluster 1: Alto Estrés (más estresado)
...
### 5.5.2. Cluster 2: Bajo Estrés (el más relajado)
...
## 6.3. Calidad del Sueño y Consumo de Cafeína

**Hipótesis:**

- H₀: No hay diferencia en la calidad del sueño según el nivel de cafeína consumido.
- H₁: Hay diferencia en la calidad del sueño según el nivel de cafeína consumido.

**Test estadístico (ANOVA):**
```{r}
df$Caffeine_Intake <- as.factor(df$Caffeine_Intake)

anova_caffeine_sleep <- aov(Sleep_Quality ~ Caffeine_Intake, data = df)
summary(anova_caffeine_sleep)
```
**Interpretación:**
El p-valor es 0.163 (> 0.05), por lo que **no hay evidencia suficiente para rechazar** la hipótesis nula. El consumo de cafeína no parece estar significativamente asociado a la calidad del sueño.

**Visualización:**
```{r}
ggplot(df, aes(x = as.factor(Caffeine_Intake), y = Sleep_Quality)) +
  geom_boxplot(fill = "skyblue") +
  labs(title = "Calidad del Sueño según Consumo de Cafeína",
       x = "Café / día", y = "Calidad del Sueño") +
  theme_minimal()
```

---

## 6.4. Horas de Trabajo y Actividad Física
...
**Interpretación:**
El p-valor es 0.0006, lo que indica una **correlación estadísticamente significativa**, aunque de magnitud baja (r = 0.12) entre las horas de trabajo y la actividad física.
...
## 6.5. Azúcar en Sangre y Colesterol
